...
/**
 * Execute a promisified function on the page.
 *
 * @param {Function} fn
 * @param {...} args
 * @param {Function} done
...
exports.evaluatePromise = function(fn/**, arg1, arg2..., done**/) {
  var args = sliced(arguments);
  var done = args[args.length-1];
  var newArgs = [fn, done].concat(args.slice(1,-1));
  if (typeof fn !== 'function') {
    return done(new Error('.evaluatePromise() fn should be a function'));
  }
  debug('.evaluatePromise() fn on the page');
  this.evaluate_promise.apply(this, newArgs);
};
