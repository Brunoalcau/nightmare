...
const DEFAULT_EVALUATE_INTERVAL = 30 * 1000;
...
function Nightmare(options) {
  ...
  options.evaluateTimeout = options.evaluateTimeout || DEFAULT_EVALUATE_INTERVAL;
...
Nightmare.prototype.end = function(done) {
  ...
  if (done && !this.running && !this.ended) {
    ...
    debug(`end() is invoked redundantly. PID ${this.proc.pid} is already ended`);
    done();
  }
