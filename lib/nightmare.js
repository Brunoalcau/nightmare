...
const DEFAULT_EVALUATE_TIMEOUT = Infinity;
...
Nightmare.prototype.end = function(done) {
  ...
  var killedTimeout = setTimeout(() => {
    ...
    killParentProcess().then(killed => {
      ...
      debug(`Error while killing parent or child process of electron PID ${this.proc.pid}: ${err.message ? err.message : err}`);
      clearKilledTimeout();
      cb(err);
    });
