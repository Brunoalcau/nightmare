...
function Nightmare(options) {
  ...
  this.queue((done) => {
    ...
    this.child.on('uncaughtException', function(stack) {
      ...
      self.state = 'failed';
      endInstance(self, self._lastRunCallback ? () => self._lastRunCallback(msg) : noop);
      //process.exit(1);
    });
    ...
    this.child.on('error', code => {
      var msg = 'Electron was ended with error code ' + code;
      debug(msg);
    ...
    this.child.on('exit', (code, signal) => {
      if (code !== 0 || this._unexpectedExitSignals.indexOf(signal)) {
        var msg = 'Electron was interrupted with error code ' + code + ' and signal ' + signal;
        debug(msg);
